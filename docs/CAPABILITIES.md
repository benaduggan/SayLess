...
- Exports and imports `.sayless-project.json` sidecars for local project backup/restore. Sidecars include recording metadata, thumbnail data, transcript, timeline clips, selected clip, normalized crop metadata, schema-v2 export settings, and schema-v4 project-audio relink metadata. Recording and added audio media remain user-controlled and are exported separately; a sidecar imported without its audio asset keeps the SHA-256 reference and asks the user to relink the original file. Older media-only entries get a default full-span project sidecar on export.
...
- Has timeline-scoped undo/redo history for non-destructive clip operations.
- Autosaves non-destructive project metadata when a recording has a local library id. Media checkpoints are reserved for explicit Apply-edits and required conversion compatibility.

Known limitations:

- Explicit Apply-edits baking still replaces the working blob by user request. Timeline edits, crop, and added/replacement audio no longer require source replacement; the unreachable destructive trim/mute/crop/audio handlers and operation protocol have been removed, and release audit prevents their standalone utilities or message paths from returning.
- Some internal state names still refer to ffmpeg even though mediabunny is the active implementation.
- Crop is stored as normalized project metadata. Added/replacement audio is decode-probed by the browser before persistence, so decodable files with missing MIME metadata are accepted while corrupt or unsupported encoded files are rejected. Valid audio is stored as a content-addressed per-recording IndexedDB asset with normalized mix/replace/loop settings exposed in the editor. Its preview controller follows resolved timeline output time, preserves the playhead during deleted-gap snaps, synchronizes seek/play/pause/playback-rate events, wraps by asset duration when looping, and becomes silent after a non-looping asset ends. Both crop and audio render in preview/export without replacing the source blob. Browser coverage includes generated WAV/M4A and the shipped encoded stereo MP3 asset; the MP3 case verifies decoded metadata and audible looped replace output, while the packaged editor proof drives that same MP3 through picker, Save, reopen, Apply-edits, and old-asset cleanup.
- Local project/timeline metadata is durable for saved local recordings. Explicit Apply-edits publishes the baked-media reference and reset project together after the blob write succeeds; ordinary timeline, crop, and project-audio changes do not create edited-media checkpoints.
...
- `src/edl/model.ts` provides v1 delete/mute EDL primitives.
- `src/edl/timeline.ts` provides a v2 ordered clip timeline with split, delete, mute, move, source/output time mapping, and transcript grouping.
- `src/edl/render.ts` plans and applies delete/mute operations through existing editor operations.
- `TranscriptPanel.tsx` can generate a transcript, seek by word, select spans, delete or mute words, reset edits, and apply edits.
- `EdlContext.tsx` loads and saves local project state for recordings opened by `localRecordingId`.
...
- During tab and region recordings, local click events are captured as bounded recording metadata. The editor turns those clicks into local zoom-moment suggestions, lets the user keep/remove them as project zoom keyframes, persists those keyframes in `.sayless-project.json` sidecars, previews them locally, and renders them into timeline-aware MP4 exports. Preview and export share the same clamped zoom viewport math, translate source-relative focus points into the active crop, and clip/retime zoom intervals through timeline deletions and reorders.
- Project schema v4 preserves the v2 normalized export settings and v3 portable crop region, then adds a bounded project-audio track with local asset id, filename/type/size, SHA-256 relink identity, volume, source gain, mix/replace mode, and loop setting. The audio editor exposes mix/replace and loop controls. The export panel exposes MP4/WebM/GIF/audio preset controls, WAV/M4A audio format selection, caption style selection, burned-in caption toggle, and checked project/transcript/WebVTT sidecar downloads while persisting normalized settings on the local project.
- The editor export panel shows a local export job panel with progress, abort-backed cancellation for timeline video/audio renders, project-audio mixing, fallback WebM conversion, GIF capture/encoding, and converted-result serialization; retry after failures or cancellation; a dismissible completion state; and a reveal action when Chrome exposes the completed download id. Renderer cancellation deterministically closes media samples, terminates GIF workers, cancels unfinished encoder outputs and FileReader work, disposes inputs, and suppresses stale download delivery. GIF capture and worker encoding share a monotonic 0–100% progress scale.
- Retrying a failed or cancelled export preserves the selected local export settings, including format, quality preset, sidecar choices, audio format, caption style/burn-in, audio-only state, and GIF bounds. Real-browser coverage cancels timeline video/audio after at least 35% progress, project audio after 90%, and post-render WebM/GIF conversion after 25%; each path suppresses cancelled delivery and immediately retries successfully. A separate 150-segment repeated-source stress case cancels a 180-second resolved timeline after 25% and fully retries to a duration-verified 179.8-second MP4. Another bounded case requires a high-entropy 960x540 WebM source to exceed 2 MB, cancels its timeline render after 35%, retries to a duration-verified five-second MP4, then uses that MP4 for cancelled/retried WebM and GIF conversion and serialization. These cases independently prove long-timeline traversal and multi-megabyte source handling, not real large-recording behavior where duration, file size, codecs, and device pressure combine.
...
- Saved zoom keyframes are rendered in editor and player preview plus timeline-aware MP4 exports, with shared viewport-framing unit coverage for center and edge click positions, crop-relative focus, output-time interval mapping, preview/export transform parity, and square/portrait/wide aspect ratios. Real-browser export pixel coverage exercises wide, square, portrait, and combined crop-plus-zoom synthetic recordings; manual visual QA is still needed across varied real recordings.
...
- Preview and media export are now connected through persisted timeline/crop/project-audio state; the editor Save panel exposes local project autosave state, clip/transcript/chapter/zoom counts, crop/audio status, export preset, and sidecar selections. Preview still uses live source seeking while export uses the multi-segment renderer. Long-timeline preview has unit coverage for bounded ticks, reordered seeks, muted clips, and deleted-gap snaps, but real playback responsiveness still needs manual QA on long recordings. Crop still needs manual visual QA with varied real dimensions and native controls; project audio needs real-input QA for playback synchronization, mix/replace gain, and relinking.
...
- `npm run test:e2e:editor-editing-proof` runs the packaged editor through non-destructive edit persistence, Apply edits, File System Access picker cancellation/retry and successful MP4 write/close, then a real completed Chrome MP4 download whose id and byte counts enable the reveal action; it is part of `npm run qa:release:auto`.
...
- `npm run qa:release:manual` verifies release-specific manual QA evidence after a human runs the real-recording checklist, including unique recording ids, exact source byte counts and dimensions, required microphone/noise descriptions, linked MP4/WebM silence-suggestion observations across distinct noise profiles, per-recording zoom observations across two click-capable sources with distinct aspect ratios, normalized edge-crop/native-control/preview/export observations across varied real dimensions, real WAV/M4A/MP3 project-audio metadata plus long-recording synchronization/gain/cancellation/relink/cleanup proof, one recording that is both at least 180 seconds and 25 MiB, cancel/retry tied to that long-and-large recording, and valid references for every cross-referenced real recording.
- The packaged-extension smoke atomically records its passed/failed browser result and the unpacked extension ID seen in a clean Chrome profile. Automated release evidence carries that ID and observation provenance; manual template synchronization prefills it, and the final verifier requires the clean-profile worksheet to report the same ID.
...
- `npm run qa:release:manual:media -- <files...>` streams user-supplied media read-only and reports exact byte counts, hashes, duration, dimensions, container/codec, and audio metadata with schema-exact recording/project-audio evidence fields. It summarizes measurable MP4/WebM, combined 180-second/25-MiB, geometry, and WAV/M4A/MP3 coverage while deliberately leaving source-role, playback, and perceptual observations manual; `--require-complete` makes incomplete measurable coverage exit nonzero, and `--output=<report.json>` atomically retains the report even on that incomplete run without overwriting an input.
- `npm run qa:release:manual:sidecars -- <files...>` validates exported WebVTT cues, transcript JSON words/kind/schema, and current v4 `.sayless-project.json` identity/timeline/export structure read-only, with hashes and copyable export fields. Coverage requires one filename-matched three-format set with agreeing JSON recording IDs, `--require-complete` makes an unmatched batch exit nonzero, and `--output=<report.json>` atomically retains the diagnostic without overwriting an inspected sidecar. The local-recordings browser harness proves strict mode accepts actual product-generated exports. Opening the text exports and importing the project remain manual evidence.
- The strict manual evidence gate requires both canonical probe reports, constrains their timestamps to the automated-QA/manual-test window, and cross-checks worksheet source, export, and project-audio filenames, byte sizes, hashes, formats, dimensions, durations, and decode metadata. It does not infer playback, perception, reveal, open, or import observations.
- `npm run qa:release:manual:measurements -- --json --write` is a guarded, atomic bridge from those strict reports to tester-selected worksheet entries. The guided progress queue routes only machine-owned hash, size, duration, dimension, container, channel, and sample-rate errors to that command after both probe stages; filenames, formats, roles, and every human observation remain tester-owned.
