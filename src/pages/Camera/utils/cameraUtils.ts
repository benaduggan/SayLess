...
interface CameraToggleRequest extends Record<string, unknown> {
  active?: boolean;
  id?: string;
}
...
export const getCameraStream = async (
  ...
  streamRef: MutableRef<MediaStream>,
  videoRef: MutableRef<HTMLVideoElement>,
  offScreenCanvasRef: MutableRef<HTMLCanvasElement>,
  offScreenCanvasContextRef: MutableRef<CanvasRenderingContext2D>,
  { onStart = () => {}, onFinish = () => {} }: CameraStreamOptions = {},
): Promise<void> => {
  ...
  try {
    ...
    const desiredVideoId =
      ...
      !Array.isArray(deviceIdConstraint)
        ? Array.isArray(deviceIdConstraint.exact)
          ? deviceIdConstraint.exact[0] || null
          : deviceIdConstraint.exact || null
        : null;
    ...
    if (setWidth && setHeight) {
      ...
      const w = isCamera || aspectRatio < 1 ? "100%" : "auto";
      const h = isCamera || aspectRatio < 1 ? "auto" : "100%";
      setWidth(w);
      setHeight(h);
    } else {
    ...
    await new Promise<void>((resolve) => {
      ...
      const timeoutId = setTimeout(() => {
        if (settled) return;
        console.warn(
          "[SayLess] Camera loadedmetadata timeout (likely virtual camera); proceeding with defaults",
          {
...
export const togglePip = (videoRef: MutableRef<HTMLVideoElement>): void => {
...
export const surfaceHandler = async (
  request: Record<string, unknown> & { surface?: string },
  videoRef: MutableRef<HTMLVideoElement>,
): Promise<void> => {
