...
const clearStaleLocks = async (): Promise<void> => {
  try {
    const {
      sendingChunks,
...
const cleanupOrphanOpfsSessions = async (): Promise<void> => {
  try {
    ...
    const recorderSession =
      all.recorderSession && typeof all.recorderSession === "object"
        ? (all.recorderSession as Record<string, unknown>)
        : null;
    const liveIds = new Set<string>();
    if (typeof recorderSession?.id === "string") liveIds.add(recorderSession.id);
    if (typeof recorderSession?.opfsSessionId === "string") {
      liveIds.add(recorderSession.opfsSessionId);
    }
