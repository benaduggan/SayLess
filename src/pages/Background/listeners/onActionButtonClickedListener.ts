...
const handleTabMessaging = async (tab: chrome.tabs.Tab): Promise<void> => {
  ...
  const preferredTabId =
    typeof recordingUiTabId === "number"
      ? recordingUiTabId
  ...
  try {
    ...
    if (targetTab) {
      ...
      await sendMessageTab(tab.id ?? null, { type: "stop-recording-tab" });
  ...
  if (!isForbidden || isPlaygroundOrSetup) {
    ...
    const onUpdated = (
      ...
      if (updatedTab.id === newTab.id && changeInfo.status === "complete") {
        ...
        setTimeout(() => {
          sendMessageTab(newTab.id ?? null, { type: "toggle-popup" });
        }, 500);
      }
    };
  ...
  chrome.action.onClicked.addListener(async (tab) => {
    try {
      ...
      if (firstTime && tab.url?.includes(chrome.runtime.getURL("setup.html"))) {
        ...
        sendMessageTab(activeTab?.id ?? null, { type: "setup-complete" });
      }
    } catch (error) {
