...
import { sendMessageRecord } from "./sendMessageRecord.ts";
import { closeOffscreenDocumentWithFlush } from "../offscreen/closeOffscreenDocumentWithFlush";
import { createRecorderOffscreen } from "../offscreen/createRecorderOffscreen.ts";
import { traceStep } from "../../utils/startFlowTrace.ts";
...
import type { DesktopCaptureRequest } from "./desktopCapture";
...
const openRecorderTab = async (
    ...
    try {
      ...
      chrome.runtime
        .sendMessage({
          ...
          error: "stream-error",
          why: "offscreen create failed: " + errorMessage(err),
  ...
  chrome.tabs.onUpdated.addListener(function _(tabId, changeInfo) {
    if (tabId === tab.id && changeInfo.status === "complete") {
      ...
      console.warn(
        "[SayLess][openRecorderTab] loadedMsg",
        JSON.stringify({
          isRegion,
          camera,
...
export const startRecorderSession = async (
  request: DesktopCaptureRequest,
  ...
  console.log("[SayLess][startRecorderSession] entered", { request, tabId });
  ...
  if (!activeTab?.id) {
    throw new Error("No active tab available for recorder session");
  }
    ...
    if (request.customRegion) {
      ...
      sendMessageRecord({
        ...
        .catch((err) =>
          endSendLoaded({
            ok: false,
            err: errorMessage(err).slice(0, 100),
          }),
        );
    } else {
      chrome.storage.local.set({
