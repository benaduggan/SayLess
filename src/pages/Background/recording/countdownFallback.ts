...
// fallback timer.
export const noteCountdownStarted = (): void => {
...
export const recoverPendingCountdownOnStartup = async (): Promise<void> => {
  try {
    ...
    console.info(
      "[SayLess][BG] recoverPendingCountdownOnStartup: SW restarted past countdown deadline, dispatching",
      { elapsedMs: elapsed },
    );
    try {
      ...
      console.error(
        "[SayLess][BG] recoverPendingCountdownOnStartup startAfterCountdown failed",
        err,
      );
    }
  } catch (err) {
...
export const initCountdownFallback = (): void => {
