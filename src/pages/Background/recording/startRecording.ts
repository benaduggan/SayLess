...
import { recordingChrome } from "./chromeTypes";
...
let _startRecordingGuardTimer: ReturnType<typeof setTimeout> | null = null;
const releaseStartRecordingGuard = (): void => {
...
export const startRecording = async (caller = "unknown"): Promise<void> => {
...
const _startRecordingInner = async (caller: string): Promise<void> => {
  ...
  const _startReads: Promise<Record<string, unknown>> = chrome.storage.local.get([
    "sandboxTab",
    "recordingToScene",
    "multiMode",
    "activeTab",
  ...
  if (alarm) {
    const seconds = parseFloat(String(alarmTime));
