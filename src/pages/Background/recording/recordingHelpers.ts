...
interface RecordingErrorRequest {
  error?: string;
  why?: string;
  errorCode?: string;
  [key: string]: unknown;
...
export const handlePip = async (started = false) => {
  ...
  if (started) {
    sendMessageTab(asTabId(activeTab), { type: "pip-started" });
  } else {
    sendMessageTab(asTabId(activeTab), { type: "pip-ended" });
  }
};

export const handleOnGetPermissions = async (request: unknown): Promise<void> => {
  const activeTab = await getCurrentTab();
  if (activeTab) {
    sendMessageTab(activeTab.id ?? null, {
...
export const handleRecordingError = async (
  ...
  if (isWarningOnly) {
    sendMessageTab(asTabId(activeTab), {
  ...
  const sandboxTabId = asTabId(sandboxTab);
  if (sandboxTabId !== null) {
    try {
      await chrome.tabs.get(sandboxTabId);
      sandboxAlive = true;
      sendMessageTab(sandboxTabId, {
  ...
  sendMessageRecord({ type: "recording-error" }).then(() => {
    ...
    if (!sandboxAlive) {
      focusTab(asTabId(activeTab));
    }
    if (request.error === "stream-error") {
      sendMessageTab(asTabId(activeTab), { type: "stream-error", errorCode });
    }
  });

  const { recordingTab } = await chrome.storage.local.get(["recordingTab"]);
  const recordingTabId = asTabId(recordingTab);
  if (recordingTabId !== null) {
    try {
      const tab = await chrome.tabs.get(recordingTabId);
      if (tab?.url?.startsWith(chrome.runtime.getURL(""))) {
        removeTab(recordingTabId);
      }
    } catch {}
  }
...
const pushStreamingData = async (dataStr: string): Promise<void> => {
...
export const videoReady = async () => {
  ...
  const backendRef: LocalRecordingBackendRef | null =
    isRecord(lastRecordingBackendRef) &&
    lastRecordingBackendRef.backend === "opfs" &&
    typeof lastRecordingBackendRef.fileName === "string"
      ? { backend: "opfs", fileName: lastRecordingBackendRef.fileName }
      : null;
  const meta = isRecord(recordingMeta) ? recordingMeta : null;
  if (backendRef) {
    try {
      ...
      await saveLocalRecordingEntry({
        ...
        createdAt: Number(meta?.startedAt) || Date.now(),
  ...
  // restarts.
  const recordingTabId = asTabId(recordingTab);
  if (backendRef && recordingTabId !== null) {
    try {
      ...
      if (isFreeRecorder) {
        ...
        await chrome.tabs.remove(recordingTabId).catch(() => {});
        chrome.storage.local.set({ recordingTab: null });
      }
    } catch (closeErr) {
      diagEvent("warning", {
        note: "eager-recorder-close failed",
        err: errorMessage(closeErr).slice(0, 120),
      });
    }
  }
  await stopRecording();
