...
import { sweepRecorderTabs } from "../recording/sweepRecorderTabs.ts";
...
const attemptStallRecovery = (recordingTabId: number): Promise<boolean> => {
...
const handleTabMessaging = async (tab: chrome.tabs.Tab | null): Promise<void> => {
  ...
  try {
    if (typeof activeTab !== "number") throw new Error("No active tab");
...
export const handleAlarm = async (alarm: chrome.alarms.Alarm): Promise<void> => {
  if (alarm.name === RECORDER_KEEPALIVE_ALARM) {
    ...
    if (typeof snap.recordingTab !== "number") return;
    const recordingTabId = snap.recordingTab;
    ...
    const age = now - (Number(snap.lastChunkAt) || 0);
    const stallLevel = Number(snap.recordingStallLevel) || 0;
    ...
    if (stallLevel >= 1) {
      ...
      const recovered = await attemptStallRecovery(recordingTabId);
      if (recovered) {
  ...
  if (alarm.name === LOCAL_PLAYBACK_ALARM) {
    ...
    if (!isRecord(localPlaybackOffer) || !localPlaybackOffer.offerId) {
