// Pair video + audio codec explicitly (w3c/mediacapture-record#194):
// Chrome mis-tags blob.type for unqualified codecs string variants.
export const MIME_TYPES = [
  "video/webm;codecs=vp9,opus",
  "video/webm;codecs=vp8,opus",
  "video/webm;codecs=h264,opus",
  "video/webm;codecs=avc1,opus",
  "video/webm",
  "video/mp4",
] as const;

export interface RecorderBitrates {
  audio: number;
  video: number;
}

export interface VideoResolution {
  width: number;
  height: number;
}

export function getBitrates(quality: string): RecorderBitrates {
  switch (quality) {
    case "4k":
      return { audio: 192000, video: 20000000 };
    case "1080p":
      return { audio: 128000, video: 12000000 };
    case "720p":
      return { audio: 128000, video: 5000000 };
    case "480p":
      return { audio: 96000, video: 3000000 };
    case "360p":
      return { audio: 96000, video: 1500000 };
    case "240p":
      return { audio: 64000, video: 1000000 };
    default:
      return { audio: 128000, video: 12000000 };
  }
}
export const VIDEO_QUALITIES: Record<string, VideoResolution> = {
  "4k": { width: 4096, height: 2160 },
  "1080p": { width: 1920, height: 1080 },
  "720p": { width: 1280, height: 720 },
  "480p": { width: 854, height: 480 },
  "360p": { width: 640, height: 360 },
  "240p": { width: 426, height: 240 },
  default: { width: 1920, height: 1080 },
};

export function getResolutionForQuality(
  qualityValue = "default",
): VideoResolution {
  return VIDEO_QUALITIES[qualityValue] || VIDEO_QUALITIES.default;
}
