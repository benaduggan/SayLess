...
export type RecorderTrack = "screen" | "audio" | "camera";
export type ChunksBackend = "opfs" | "idb";

export const TRACK_KEY_PREFIX: Record<RecorderTrack, string> = {
  screen: "chunk_",
  audio: "audio_chunk_",
  camera: "camera_chunk_",
};

export const TRACK_IDB_INSTANCE: Record<RecorderTrack, string> = {
  screen: "chunks",
  audio: "audioChunks",
  camera: "cameraChunks",
};
...
const idbInstance = (track: RecorderTrack): LocalForage =>
...
// chunk is written.
const probeOpfs = async (store: OpfsKvStore): Promise<void> => {
...
export const chooseChunksStore = async ({
  ...
  | { store: OpfsKvStore; backend: "opfs"; initMs: number }
  | { store: LocalForage; backend: "idb"; initMs: number }
> => {
...
export const openExistingChunksStore = ({
  sessionId,
  track,
  backend,
}: {
