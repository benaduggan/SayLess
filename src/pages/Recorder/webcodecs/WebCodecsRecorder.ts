...
import type { MkvMuxerWrapper } from "./MkvMuxerWrapper";

type RecorderMuxer = Mp4MuxerWrapper | MkvMuxerWrapper;
type RecorderTimer = ReturnType<typeof setTimeout>;
type SelectedVideoConfig = {
...
type TimedReadResult<T> = {
  ...
  value?: T | null;
  timedOut?: boolean;
  isSyntheticTimeout?: boolean;
};

interface RecorderError extends Error {
  code?: string;
  detail?: Record<string, unknown>;
  codec?: string;
  swRetryReason?: string | null;
  cooldownRetried?: boolean;
  finalized?: boolean;
}
...
export interface WebCodecsRecorderOptions {
  ...
  firstChunkWatchdogMs?: number;
  midStreamWatchdogMs?: number;
  encoderMaxQueueSize?: number;
  audioEncoderMaxQueueSize?: number;
  maxEncoderReclaims?: number;
  reclaimRebuildThrottleMs?: number;
  reclaimResetAfterMs?: number;
  maxAudioSampleRateRebuilds?: number;
  sleepGapMs?: number;
  encoderConfigureCooldownMs?: number;
  onChunk: (chunk: Uint8Array, timestampUs: number | null) => void | Promise<void>;
  onError?: (error: unknown) => void;
  onStop?: () => void | Promise<void>;
  onFinalized?: (result: Record<string, unknown>) => void | Promise<void>;
...
let _mp4MuxerWrapperPromise: Promise<typeof Mp4MuxerWrapper> | null = null;
const loadMp4MuxerWrapper = () => {
  if (!_mp4MuxerWrapperPromise) {
    _mp4MuxerWrapperPromise = import("./Mp4MuxerWrapper").then((m) => m.Mp4MuxerWrapper);
  }
  return _mp4MuxerWrapperPromise;
};

let _mkvMuxerWrapperPromise: Promise<typeof MkvMuxerWrapper> | null = null;
const loadMkvMuxerWrapper = () => {
  if (!_mkvMuxerWrapperPromise) {
    _mkvMuxerWrapperPromise = import("./MkvMuxerWrapper").then((m) => m.MkvMuxerWrapper);
  }
...
export class WebCodecsRecorder {
  ...
  videoTrack: (MediaStreamTrack & MediaStreamVideoTrack) | null;
  audioTrack: (MediaStreamTrack & MediaStreamAudioTrack) | null;
  videoProcessor: MediaStreamTrackProcessor<VideoFrame> | null;
  audioProcessor: MediaStreamTrackProcessor<AudioData> | null;
  ...
  muxer: RecorderMuxer | null;
  running: boolean;
  actualWidth: number;
  actualHeight: number;
  targetWidth: number;
  targetHeight: number;
  videoTimestampOffsetUs: number | null;
  videoFallbackStartMs: number | null;
  selectedVideoCodec: string | null;
  actualVideoCodec: string | null;
  firstVideoFrame: VideoFrame | undefined;
  frameCount: number;
  _videoFrameIndex: number;
  _videoStartUs: number | null;
  _frameDurationUs: number | null;
  _lastKeyFrameIndex: number;
  _keyFrameIntervalFrames: number;
  audioSamplesWritten: number;
  audioElapsedUs: number;
  audioSampleRate: number | null;
  audioChannelCount: number | null;
  _firstAudioFrameSampleRate: number | null;
  resizeCanvas: OffscreenCanvas | HTMLCanvasElement | null;
  resizeCtx: OffscreenCanvasRenderingContext2D | CanvasRenderingContext2D | null;
  justResumed: boolean;
  paused: boolean;
  pauseStartUs: number | null;
  totalPausedDurationUs: number;
  _startPromise: Promise<boolean> | null;
  _startResolve: ((started: boolean) => void) | null;
  _prebufferedAudio: AudioData[];
  _audioReady: boolean;
  _stopping: boolean;
  _videoLoopPromise: Promise<void> | null;
  _audioLoopPromise: Promise<void> | null;
  _failureReported: boolean;
  _videoLoopError: RecorderError | null;
  _firstChunkSeen: boolean;
  _firstChunkWatchdog: RecorderTimer | null;
  _firstChunkWatchdogMs: number;
  _lastChunkAt: number | null;
  _midStreamWatchdog: RecorderTimer | null;
  _midStreamWatchdogMs: number;
  _lastFailureCode: string | null;
  _encoderMaxQueueSize: number;
  _forceNextKeyframe: boolean;
  _droppedForBackpressureCount: number;
  _audioEncoderMaxQueueSize: number;
  _droppedAudioForBackpressureCount: number;
  _peakVideoEncodeQueueSize: number;
  _peakAudioEncodeQueueSize: number;
  _maxEncoderReclaims: number;
  _reclaimRebuildThrottleMs: number;
  _lastVideoReclaimRebuildAt: number | null;
  _lastAudioReclaimRebuildAt: number | null;
  _encoderReclaimCount: number;
  _audioEncoderReclaimCount: number;
  _midStreamStallRebuilds: number;
  _lastVideoReclaimAt: number | null;
  _lastAudioReclaimAt: number | null;
  _reclaimResetAfterMs: number;
  _encodersClosedForPause: boolean;
  _pauseResumeChain: Promise<void>;
  _audioDeviceChangePending: boolean;
  _audioSampleRateMismatchRebuilds: number;
  _maxAudioSampleRateRebuilds: number;
  _lastFrameArrivalAt: number | null;
  _postSleepRecoveries: number;
  _sleepGapMs: number;
  _activeVideoConfig: VideoEncoderConfig | null;
  _activeVideoCodecLabel: string | null;
  _activeAudioConfig: AudioEncoderConfig | null;
  _videoEncoderOutputHandler: EncodedVideoChunkOutputCallback | null;
  _videoEncoderErrorHandler: WebCodecsErrorCallback | null;
  _audioEncoderOutputHandler: EncodedAudioChunkOutputCallback | null;
  _audioEncoderErrorHandler: WebCodecsErrorCallback | null;
  _visibilityChangeCount: number;
  _lastVisibilityChangeAt: number | null;
  _lastVisibilityState: DocumentVisibilityState | null;
  _visibilityChangeListener: (() => void) | null;
  _deviceChangeCount: number;
  _lastDeviceChangeAt: number | null;
  _deviceChangeListener: (() => void) | null;
  _decoderConfigEmitted: boolean;
  _framesFed: number;
  _chunksOut: number;
  _framesFromMSTP: number;
  _staticFrameSyntheticCount: number;
  _syntheticFirstFrameCount: number;
  _firstChunkAt: number | null;
  _encoderConstructCount: number;
  _videoEncoderStateAtStop: CodecState | "destroyed" | "unknown" | null;
  _videoCodecConfigsTried: Array<Record<string, unknown>>;
  _displaySurface: string | null;
  _frameRateRequested: number | null;
  _frameRateActual: number | null;
  _frameWidthActual: number | null;
  _frameHeightActual: number | null;
  _audioFlushMs?: number;
  _videoFlushMs?: number;
  _audioRebuildUnsupportedRates!: Set<number> | null;
  _didCooldownRetry?: boolean;
  _didSwRetry?: boolean;
  _swRetryReason?: string | null;
  _finalizingFromFatal?: boolean;
  _finalizeOnFatalRan?: boolean;
  _forcedTrackEndTimer?: RecorderTimer | null;
  _startAlignTimer?: RecorderTimer | null;
  _pendingAudioConfig?: AudioEncoderConfig | null;

  constructor(stream: MediaStream, options: WebCodecsRecorderOptions) {
    this.stream = stream;
    ...
    this._firstChunkWatchdogMs = Number.isFinite(options.firstChunkWatchdogMs)
      ...
      : Number.isFinite(/** @type {any} */ (globalThis).__screenityFirstChunkWatchdogMsForTests)
        ? /** @type {any} */ (globalThis).__screenityFirstChunkWatchdogMsForTests!
        : 12000;
    ...
    this._midStreamWatchdogMs = Number.isFinite(options.midStreamWatchdogMs)
      ...
      : Number.isFinite(/** @type {any} */ (globalThis).__screenityMidStreamWatchdogMsForTests)
        ? /** @type {any} */ (globalThis).__screenityMidStreamWatchdogMsForTests!
        : 15000;
    ...
    this._reclaimRebuildThrottleMs = Number.isFinite(options.reclaimRebuildThrottleMs)
      ? options.reclaimRebuildThrottleMs!
      : Number.isFinite(/** @type {any} */ (globalThis).__screenityReclaimRebuildThrottleMsForTests)
        ? /** @type {any} */ (globalThis).__screenityReclaimRebuildThrottleMsForTests!
        : 2000;
    ...
    this._reclaimResetAfterMs = Number.isFinite(options.reclaimResetAfterMs)
      ...
      : Number.isFinite(/** @type {any} */ (globalThis).__screenityReclaimResetAfterMsForTests)
    ...
    this._maxAudioSampleRateRebuilds = Number.isFinite(options.maxAudioSampleRateRebuilds)
    ...
    this._sleepGapMs = Number.isFinite(options.sleepGapMs) ? options.sleepGapMs! : 5000;
  ...
  async _probeRealResolution(): Promise<{ width: number; height: number }> {
    ...
    try {
      const settings =
        this.videoTrack && typeof this.videoTrack.getSettings === "function"
    ...
    if (!this.videoTrack) throw new Error("Cannot probe missing video track");
  ...
  async start() {
    ...
    this._visibilityChangeCount = 0;
    this._lastVisibilityChangeAt = null;
    this._lastVisibilityState = typeof document !== "undefined" ? document.visibilityState : null;
    this._deviceChangeCount = 0;
    this._lastDeviceChangeAt = null;
    ...
    try {
      this.videoTrack =
        (this.stream.getVideoTracks()[0] as
          | (MediaStreamTrack & MediaStreamVideoTrack)
          | undefined) || null;
      this.audioTrack =
        (this.stream.getAudioTracks()[0] as
          | (MediaStreamTrack & MediaStreamAudioTrack)
          | undefined) || null;

      this.log("[WCR] videoTrack", this.videoTrack, "state=", this.videoTrack?.readyState);
      this.log("[WCR] audioTrack", this.audioTrack, "state=", this.audioTrack?.readyState);
      ...
      try {
        ...
        let videoConfig: SelectedVideoConfig | null = null;
        if (overrideConfig && typeof overrideConfig.codec === "string") {
          const config: VideoEncoderConfig = {
            ...overrideConfig,
            codec: overrideConfig.codec,
          ...
          try {
            ...
            if (support?.supported) {
              const effectiveCodec = (support.config || config).codec || overrideConfig.codec || "";
              let cc;
        ...
        if (this.containerKind === "webm") {
          ...
          this.muxer = new MkvMuxerWrapper({
            ...
            audioBitrate: this.options.audioBitrate || 0,
        ...
        this._startAlignTimer = setTimeout(() => {
          ...
          if (globalThis.__screenityForceWebCodecsError) {
            ...
            setTimeout(() => {
              this.options.onError?.(new Error("e2e-forced-webcodecs-error"));
            }, 0);
            return;
          }
          ...
          const endAfterMs = Number(globalThis.__screenityForceTrackEndAfterMs);
          if (this.videoTrack && Number.isFinite(endAfterMs) && endAfterMs > 0) {
  ...
  async stop() {
      ...
      try {
        await Promise.race([
          ...
          new Promise((_, reject) => {
            timed = setTimeout(() => reject(new Error(`${kind}-flush-timeout`)), 15000);
          }),
        ]);
      } finally {
    ...
    if (
      ...
      try {
        ...
        const targetEndUs = Math.max(holdStartUs + this._frameDurationUs, audioEndUs + cushionUs);
        ...
        for (let k = 0; k < framesNeeded; k += 1) {
          ...
          try {
            this.videoEncoder.encode(hold, { keyFrame: false });
            this._framesFed += 1;
          } finally {
            hold.close();
          }
        ...
        try {
          await Promise.race([
            ...
            new Promise((_, reject) => {
              holdTimed = setTimeout(() => reject(new Error("trailing-hold-flush-timeout")), 5000);
            }),
          ]);
        } finally {
    ...
    if (shouldFailZeroChunksAtStop(this._firstChunkSeen, this.frameCount)) {
      ...
      if (!this._failureReported) {
        const zeroChunkErr = (this._videoLoopError ||
          new Error(
    ...
    if (this.muxer && !this._failureReported) {
      ...
      try {
        ...
        endFinalize({ ok: false, err: errorMessage(err).slice(0, 80) });
    ...
    if (this.options.onFinalized) {
      ...
      try {
        ...
        await this.options.onFinalized({
          ...
          swRetry: this._didSwRetry ? { reason: this._swRetryReason || null } : null,
          lastFailureCode: this._lastFailureCode,
          diag: this.getDiagSnapshot(),
        });
        endOnFinalized({ ok: true });
      } catch (err) {
        endOnFinalized({ ok: false, err: errorMessage(err).slice(0, 80) });
  ...
  getDiagSnapshot() {
    ...
    if (this._firstChunkAt != null && this._videoStartUs != null) {
      firstChunkLatencyMs = Math.round(this._firstChunkAt - this._videoStartUs / 1000);
    }
    return {
      videoCodecString: this.actualVideoCodec || null,
      hardwareAccelerationActual: this._activeVideoConfig?.hardwareAcceleration || "no-preference",
  ...
  pause() {
    this._pauseResumeChain = this._pauseResumeChain.catch(() => {}).then(() => this._pauseImpl());
    return this._pauseResumeChain;
  }

  resume() {
    this._pauseResumeChain = this._pauseResumeChain.catch(() => {}).then(() => this._resumeImpl());
    return this._pauseResumeChain;
  }
  ...
  async _resumeImpl() {
    ...
    this.totalPausedDurationUs += nowUs - (this.pauseStartUs ?? nowUs);
  ...
  async cleanup() {
    ...
    const cancelWithTimeout = async (
      ...
      if (!reader) return;
      try {
        await Promise.race([reader.cancel(), new Promise((resolve) => setTimeout(resolve, 500))]);
      } catch {}
      try {
        reader.releaseLock();
      } catch {}
    };
    await Promise.all([cancelWithTimeout(this.videoReader), cancelWithTimeout(this.audioReader)]);
  ...
  _reportFailure(code: string, originalError: unknown) {
    ...
    if (!err.detail) {
      err.detail = {
        ...
        swRetry: this._didSwRetry ? { reason: this._swRetryReason || null } : null,
        videoLoopError: this._videoLoopError ? this._videoLoopError.message : null,
        ...
        visibility: {
          ...
          stateNow: typeof document !== "undefined" ? document.visibilityState : null,
        },
        reclaims: {
  ...
  _armFirstChunkWatchdog() {
    ...
    this._firstChunkWatchdog = setTimeout(() => {
      ...
      this.warn(`[WCR] no encoded video chunk within ${this._firstChunkWatchdogMs}ms`);
      this._reportFailure(
        "webcodecs-no-first-chunk",
        new Error(`WebCodecs produced no encoded video within ${this._firstChunkWatchdogMs}ms`),
      );
  ...
  async initVideoEncoder(config: VideoEncoderConfig, codecLabel: string) {
    ...
    const videoEncoderOutput: EncodedVideoChunkOutputCallback = (chunk, meta) => {
      ...
      if (descForFix && typeof codecForFix === "string" && codecForFix.startsWith("avc")) {
        const u8 = toUint8Array(descForFix);
      ...
      if (!this._decoderConfigEmitted && typeof this.options.onDecoderConfig === "function") {
        const desc = meta?.decoderConfig?.description;
    ...
    const videoEncoderError: WebCodecsErrorCallback = (err) => {
    ...
    if (typeof globalThis !== "undefined") {
      ...
      g.__screenityFireVideoEncoderError = (customMsg) => {
        const synthetic = new DOMException(
    ...
    const tryConfigureWith = (cfg: VideoEncoderConfig) => {
      // E2E hook: force one HW configure() to throw QuotaExceededError so
      // tests can exercise the prefer-software retry path. One-shot.
      const hwPref = cfg.hardwareAcceleration === "prefer-hardware" || !cfg.hardwareAcceleration;
      ...
      if (
        ...
        const forced = new Error("Forced HW configure quota error for testing");
      ...
      if (
        ...
        const forced = new Error("Forced SW configure quota error for testing");
        forced.name = "QuotaExceededError";
        throw forced;
      }
    ...
    const isContention = (err: unknown) => {
    ...
    const buildBusyError = (cause: unknown, codec: string): RecorderError => {
      // the raw DOMException ("Codec reclaimed due to inactivity") is opaque;
      // give a user-facing message. errorCodes.ts still maps to REC_START_CODEC.
    ...
    try {
      ...
      const isHwPreferred =
        config.hardwareAcceleration === "prefer-hardware" || !config.hardwareAcceleration;
      const errMsg = errorMessage(err);
      const isContentionError = isContention(err);
      if (isHwPreferred && !this._didSwRetry) {
        this._didSwRetry = true;
        this._swRetryReason = isContentionError ? "encoder-quota" : "configure-failed";
        this.warn("[WCR] HW VideoEncoder.configure failed, retrying prefer-software", {
          reason: this._swRetryReason,
          codec: codecLabel,
          message: errMsg,
        });
        rebuildEncoder();
        ...
        try {
          ...
          if (isContention(swErr) && isContentionError && !this._didCooldownRetry) {
            this._didCooldownRetry = true;
            const cooldownMs = Number.isFinite(this.options.encoderConfigureCooldownMs)
            ...
            try {
              tryConfigureWith(config);
              this.log("[WCR] VideoEncoder configured after cooldown retry", { codec: codecLabel });
              this._activeVideoConfig = config;
              return;
            } catch (retryErr) {
              this.warn("[WCR] cooldown HW retry failed; trying SW once more", {
                codec: codecLabel,
              ...
              try {
                tryConfigureWith(swConfig);
                this.log("[WCR] VideoEncoder configured (SW after cooldown)", {
                  codec: codecLabel,
                });
                this._activeVideoConfig = swConfig;
                return;
          ...
          const tagged = new Error(
            `VideoEncoder.configure failed for ${codecLabel} on both HW and SW: ${errorMessage(swErr)}`,
          ) as RecorderError;
          tagged.cause = swErr;
  ...
  async initAudioEncoder(config: AudioEncoderConfig | null) {
    if (!config) return;

    const audioEncoderOutput: EncodedAudioChunkOutputCallback = (chunk, meta) => {
    ...
    const audioEncoderError: WebCodecsErrorCallback = (err) => {
    ...
    try {
      ...
      const tagged = new Error(
        `AudioEncoder.configure failed: ${errorMessage(err)}`,
      ) as RecorderError;
      tagged.cause = err;
  ...
  _maybeFixAacDescription(
    meta: EncodedAudioChunkMetadata | undefined,
  ): EncodedAudioChunkMetadata | null {
    const dc = meta?.decoderConfig;
    if (!dc) return null;
    const codec = dc.codec || this._activeAudioConfig?.codec || "";
    if (typeof codec !== "string" || !codec.startsWith("mp4a.40.")) return null;
    ...
    const sampleRate =
      dc.sampleRate || this._activeAudioConfig?.sampleRate || this.audioSampleRate || 48000;
    const channels =
      dc.numberOfChannels ||
    ...
    const synthesized = this._synthesizeAacDescription(
      Number.isFinite(objectTypeFromCodec) && objectTypeFromCodec > 0 ? objectTypeFromCodec : 2,
      sampleRate,
      channels,
    );
  ...
  _synthesizeAacDescription(
    ...
    const byte0 = ((objectType & 0x1f) << 3) | ((freqIndex >> 1) & 0x07);
  ...
  _handleVideoEncoderError(err: unknown, forceReclaim = false) {
    const msg = errorMessage(err);
    ...
    if (isReclaim && canRecover && this._encoderReclaimCount < this._maxEncoderReclaims) {
      ...
      if (this._activeVideoConfig && this._rebuildVideoEncoder(this._activeVideoConfig)) {
        this._forceNextKeyframe = true;
        return;
      }
    }
    ...
    if (preFirstChunk && canRecover && isHwPreferred && !this._didSwRetry) {
      ...
      this.warn("[WCR] VideoEncoder errored before first chunk, retrying prefer-software", {
        codec: this._activeVideoCodecLabel,
        message: msg,
      });
      const activeVideoConfig = this._activeVideoConfig;
    ...
    if (this._firstChunkSeen && this.running && !this._stopping && !this._finalizeOnFatalRan) {
      this._finalizeOnFatalRan = true;
      try {
        (err as RecorderError).finalized = true;
      } catch {}
  ...
  async _finalizeOnFatal(originalErr: unknown) {
  ...
  _handleAudioEncoderError(err: unknown) {
  ...
  _rebuildVideoEncoder(config: VideoEncoderConfig): boolean {
    try {
      this.videoEncoder?.close();
    } catch {}
    try {
      let nextEncoder: VideoEncoder | null = null;
      const output = this._videoEncoderOutputHandler;
      const handleError = this._videoEncoderErrorHandler;
      if (!output || !handleError) return false;
  ...
  _rebuildAudioEncoder(config: AudioEncoderConfig): boolean {
    ...
    const output = this._audioEncoderOutputHandler;
    const handleError = this._audioEncoderErrorHandler;
    if (!output || !handleError) return false;
    try {
      this.audioEncoder?.close();
    } catch {}
    try {
      let nextEncoder: AudioEncoder | null = null;
      ...
      if (priorConfig) {
        try {
          let restoreEncoder: AudioEncoder | null = null;
          ...
          this.err("[WCR] AudioEncoder restore after failed rebuild also failed:", restoreErr);
        }
      }
      return false;
    }
  ...
  _retimeAudioData(audioData: AudioData, timestampUs: number): AudioData {
    if (!audioData || typeof AudioData === "undefined") return audioData;
    const frames = typeof audioData.numberOfFrames === "number" ? audioData.numberOfFrames : 0;
    const channels =
      typeof audioData.numberOfChannels === "number" ? audioData.numberOfChannels : 0;
    const sampleRate = audioData.sampleRate || this.audioSampleRate || 48000;
    if (frames <= 0 || channels <= 0 || !Number.isFinite(sampleRate)) {
      return audioData;
    }
    if (Math.round(audioData.timestamp || 0) === Math.round(timestampUs || 0) && audioData.format) {
      return audioData;
    }
    ...
    try {
      ...
      for (let planeIndex = 0; planeIndex < channels; planeIndex += 1) {
        const plane = samples.subarray(planeIndex * frames, (planeIndex + 1) * frames);
  ...
  async chooseVideoEncoderConfig({
    width,
    height,
    fps,
    bitrate,
    ...
    for (const c of candidates) {
      ...
      const resolvedConfig = supported?.supported && supported.config ? supported.config : config;
  ...
  async readVideoLoop() {
    ...
    try {
      while (this.running) {
        ...
        const g = /** @type {any} */ (globalThis);
        const forcedGap =
          typeof g.__screenityForceSleepGapMs === "number" ? g.__screenityForceSleepGapMs : 0;
        if (forcedGap > 0) {
          g.__screenityForceSleepGapMs = 0;
          this._lastFrameArrivalAt = _nowLoopTick - forcedGap;
        }
        const lastFrameArrivalAt = this._lastFrameArrivalAt;
        if (
          shouldTriggerSleepRecovery(
            _nowLoopTick,
            lastFrameArrivalAt,
          ...
          this.warn("[WCR] post-sleep gap detected; resetting watchdog", {
            gapMs: Math.round(_nowLoopTick - (lastFrameArrivalAt ?? _nowLoopTick)),
        ...
        const readResult: TimedReadResult<VideoFrame> = await Promise.race([
          this.videoReader
            .read()
          ...
          new Promise<TimedReadResult<VideoFrame>>((resolve) =>
            setTimeout(
              () =>
                resolve({
                  done: false,
        ...
        if (readResult.isSyntheticTimeout) {
          ...
          this.resizeCtx!.drawImage(frame, 0, 0, this.targetWidth, this.targetHeight);
        ...
        const elapsedUs = Math.max(
          0,
          nowUs - (this._videoStartUs ?? nowUs) - (this.totalPausedDurationUs || 0),
        );
        ...
        for (let i = startIndex; i <= endIndex; i += 1) {
          ...
          if (this.paused || !this.videoEncoder || this.videoEncoder.state === "closed") {
            resized.close();
            this._videoFrameIndex = i + 1;
            this._forceNextKeyframe = true;
            continue;
          ...
          try {
            try {
              ...
              if (encErr instanceof Error && encErr.name === "QuotaExceededError") {
                ...
                this._forceNextKeyframe = true;
                this._videoFrameIndex = i + 1;
                this.warn("[WCR] encode() QuotaExceededError; dropping frame as backpressure");
                continue;
              }
              throw encErr;
            }
      ...
      this._videoLoopError = err instanceof Error ? (err as RecorderError) : new Error(String(err));
    }
    ...
    if (this.running) {
      ...
      if (this.frameCount === 0) {
        ...
        this._reportFailure(
          ...
          this._videoLoopError || new Error("WebCodecs video loop produced zero frames"),
        );
      } else {
        // Some frames encoded: a real, if short, partial recording
        // exists. Keep it; finalize() ships what we have.
        this.warn("[WCR] video loop exited early while running; partial recording kept");
      }
    }
  }
  ...
  async readAudioLoop() {
    ...
    const encodeAudioData = (audioData: AudioData) => {
      ...
      if (
        ...
        incomingSampleRate !== this.audioSampleRate &&
        !rateUnsupported &&
        this._audioSampleRateMismatchRebuilds < this._maxAudioSampleRateRebuilds
      ) {
        ...
        try {
          diagForward("recorder-audio-rebuild-unsupported", {
            rate: incomingSampleRate,
            keptRate: this.audioSampleRate,
            codec: (this._activeAudioConfig && this._activeAudioConfig.codec) || null,
          });
        } catch {}
      }
      ...
      const aQueueSize = audioEncoder.encodeQueueSize;
      ...
      if (shouldDropAudioForBackpressure(aQueueSize, this._audioEncoderMaxQueueSize)) {
      ...
      try {
        ...
        audioEncoder.encode(audioForEncode);
      } catch (encErr) {
        ...
        if (encErr instanceof Error && encErr.name === "QuotaExceededError") {
          ...
          this.audioSamplesWritten += frames;
          this.audioElapsedUs = nextElapsedUs;
          this.warn("[WCR] audio encode() QuotaExceededError; dropping audio data");
          return;
        }
        throw encErr;
      } finally {
    ...
    const makeSilentAudioData = (durationMs: number): AudioData => {
    ...
    try {
      while (this.running) {
        const readResult: TimedReadResult<AudioData> = await Promise.race([
          this.audioReader
            .read()
            .then((v) => ({ ...v, timedOut: false }))
            .catch(() => ({ done: true, timedOut: false })),
          new Promise<TimedReadResult<AudioData>>((r) =>
            setTimeout(() => r({ value: null, done: false, timedOut: true }), SILENCE_TIMEOUT_MS),
          ),
        ]);

        if (readResult.timedOut) {
          const trackEnded = !this.audioTrack || this.audioTrack.readyState === "ended";
          if (
            !this._audioReady ||
            this.paused ||
            trackEnded ||
          ...
          try {
            ...
            if (paddedSilenceCount === 1 || paddedSilenceCount % 20 === 0) {
              this.log("[WCR] audio source quiet, padding silence", paddedSilenceCount);
            }
          } catch (err) {
        ...
        if (this._firstAudioFrameSampleRate == null && typeof audioData.sampleRate === "number") {
            ...
            chrome.storage.local.get(["lastRecordingAudioSnapshot"], (res) => {
              ...
              chrome.storage.local.set({
                lastRecordingAudioSnapshot: {
                  ...
                  firstFrameChannels: audioData.numberOfChannels ?? null,
        ...
        try {
          if (this._prebufferedAudio.length) {
            while (this._prebufferedAudio.length) {
              ...
              try {
                if (buffered) encodeAudioData(buffered);
              } finally {
                buffered?.close();
              }
            }
    ...
    if (paddedSilenceCount > 0) {
      this.log("[WCR] audio loop exit, total silence chunks padded:", paddedSilenceCount);
      try {
        chrome.storage.local.get(["lastRecordingAudioSnapshot"], (res) => {
          const prev = res?.lastRecordingAudioSnapshot || {};
          chrome.storage.local.set({
