...
export interface EncoderPrewarmConfig {
  width: number;
  height: number;
  codec: string;
  bitrate?: number;
...
export interface EncoderPrewarmHandle {
  close(): Promise<void>;
  readonly chunks: number;
  readonly ok: boolean;
}

interface EncoderPrewarmState {
  chunks: number;
  ...
  keepAliveIndex: number;
  handle?: EncoderPrewarmHandle;
}

let _activeWarmer: EncoderPrewarmState | null = null;

const errorMessage = (error: unknown): string =>
  error instanceof Error ? error.message : String(error);

const computeFramerate = (config: EncoderPrewarmConfig): number => {
  const fps = Number(config?.framerate);
  if (!Number.isFinite(fps) || fps <= 0 || fps > 240) return 30;
  return Math.round(fps);
};

const synthesizeAndEncode = (
 ...
 */
export const startEncoderPrewarm = async (
  ...
  const state: EncoderPrewarmState = {
    chunks: 0,
    encoder: null,
    canvas: null,
    ctx: null,
  ...
  try {
    state.encoder = new VideoEncoder({
      ...
      error: (err) => {
        perfMark("Recorder.encoderPrewarm.encoderError", {
          msg: errorMessage(err).slice(0, 120),
        });
      },
    });
    ...
    endStart({ ok: false, error: errorMessage(err).slice(0, 100) });
...
export const getActiveEncoderPrewarm = (): EncoderPrewarmHandle | null =>
  _activeWarmer?.handle || null;

/** Synchronously close any active prewarm. Safe at any time. */
export const closeActiveEncoderPrewarm = async (): Promise<void> => {
