...
const Recorder = (): React.JSX.Element => {
  const permissionStatusesRef = useRef<PermissionStatus[]>([]);
  const checkingRef = useRef(false);
  const pendingCheckRef = useRef<PublishOptions | null>(null);

  const publishPermissions = useCallback(async (options: PublishOptions = {}) => {
  ...
  useEffect(() => {
    ...
    Promise.all(
      ...
      permissionStatusesRef.current = statuses.filter(
        (status): status is PermissionStatus => status !== null,
      );
    ...
    const handleMessage = (event: MessageEvent): void => {
      ...
      if (event.data?.type === "screenity-get-permissions") {
        ...
        publishPermissions({
          request: true,
          requestedTypes: normalizeRequestedTypes(event.data.mediaTypes),
        });
      }
    };
