...
// (recorderKeepalive.ts) for its own historical reasons; this helper
...
export interface TabKeepaliveHandle {
  pcTick?: ReturnType<typeof setInterval>;
  pcTrack?: MediaStreamTrack;
...
const startLoopbackPriorityBoost = (state: TabKeepaliveHandle): void => {
    ...
    const tick = setInterval(() => {
      try {
        ...
        if (stream && typeof stream.getVideoTracks === "function") {
          ...
          if (t && "requestFrame" in t) {
            (t as CanvasCaptureMediaStreamTrack).requestFrame();
          }
        }
      } catch {}
    }, 2000);
...
const startLock = (state: TabKeepaliveHandle): void => {
  try {
...
const startMediaSession = (state: TabKeepaliveHandle): void => {
  try {
...
export const startTabKeepalive = (): TabKeepaliveHandle | null => {
  if (typeof window === "undefined") return null;
  const state: TabKeepaliveHandle = {};
