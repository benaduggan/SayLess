...
export interface AudioContextWatchdogStats {
  interruptedCount: number;
  interruptedTotalMs: number;
  sawInterrupted: boolean;
}

type ExtendedAudioContext = Omit<AudioContext, "state"> & {
...
export function attachAudioContextWatchdog(
  aCtx: ExtendedAudioContext | null | undefined,
  label: string,
): AudioContextWatchdogStats {
  const stats = {
    interruptedCount: 0,
