...
import { getUserMediaWithFallback } from "./mediaDeviceFallback.ts";

interface AudioStreamLogger {
  debug?: (...args: unknown[]) => void;
  warn?: (...args: unknown[]) => void;
}

interface StartAudioStreamOptions {
  bailOnNone?: boolean;
  bluetoothDiag?: boolean;
  toastOnBlocked?: boolean;
  logger?: AudioStreamLogger | null;
}

interface StoredAudioDevice {
  deviceId?: string;
  label?: string;
}

interface AudioChromeApi {
  storage: {
    local: {
      get: (keys: string[]) => Promise<{
        defaultAudioInputLabel?: string;
        audioinput?: StoredAudioDevice[];
      }>;
...
const chromeApi = (): AudioChromeApi =>
  (globalThis as typeof globalThis & { chrome: AudioChromeApi }).chrome;

const buildAudioConstraints = (id: string | null): MediaTrackConstraints => {
  const base = {
...
const recordMicInputDiagnostics = (
  stream: MediaStream,
  { bluetoothDiag = false }: { bluetoothDiag?: boolean } = {},
): void => {
...
export async function startAudioStream(
  ...
  const audioStreamOptions: MediaStreamConstraints & { mimeType: string } = {
    mimeType: "video/webm;codecs=vp8,opus",
    audio: buildAudioConstraints(useExact ? id : null),
  };

  const { defaultAudioInputLabel, audioinput } = await chromeApi().storage.local.get([
