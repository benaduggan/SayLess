...
import { AUDIO_DIAGNOSTIC_KEYS, buildAudioDiagnosticsContext } from "./audioDiagnostics.ts";
...
const sanitizeError = (err: unknown): string => {
  ...
  const str =
    typeof err === "string"
      ? err
      : asRecord(err).message || asRecord(err).errorCode || JSON.stringify(err);
  return (
    String(str)
...
export const buildSupportContext = async (
  ...
  if (opts.includeRecordingState) {
    try {
      ...
      if (store.fastRecorderInUse != null) ctx.fast = store.fastRecorderInUse ? "1" : "0";
      ...
      if (store.lastRecordingError) {
        ctx.lastErr = sanitizeError(
          asRecord(store.lastRecordingError).why || asRecord(store.lastRecordingError).error,
        );
      ...
      if (backendRef.fileName) {
        ctx.finalized = store.lastRecordingFinalizedFileName === backendRef.fileName ? "1" : "0";
      }
      ...
      if (store.freeRecorderSession) {
        ...
        if (session.chunkCount != null) ctx.chunks = String(session.chunkCount);
        if (session.status) ctx.recSessStatus = String(session.status).slice(0, 30);
      }
      ...
      if (store.fastRecorderValidation) {
        ...
        if (!validation.ok && Array.isArray(validation.reasons)) {
          ctx.validationReason = String(validation.reasons[0] || "").slice(0, 60);
        }
        const validationSize = asRecord(validation.details).size;
        if (typeof validationSize === "number") {
          // MB rounded; keeps the URL short.
          ctx.fileMb = String(Math.round(validationSize / (1024 * 1024)));
        }
      }
      ...
      if (trackEndReason) {
        ctx.trackEndReason = String(trackEndReason).slice(0, 60);
      }
      ...
      if (editorErrorCode) {
        ctx.editorErr = String(editorErrorCode).slice(0, 60);
      }
...
export const supportContextQuery = async (opts: SupportContextOptions = {}): Promise<string> => {
