...
const ToolbarWrap = () => {
  ...
  const DragRef = React.useRef<Rnd | null>(null);
  ...
  const [transparent, setTransparent] = React.useState<string | false>(false);
    ...
    const handleMouseDown = (e: MouseEvent) => {
      if (contentState.toolbarHover && contentState.hideUI) {
        ...
        if (
          ...
          (contentState.shadowRef.contains(target) ||
            contentState.shadowRef === target ||
            contentState.shadowRef === target?.parentNode)
        )
          return;
  ...
  useEffect(() => {
    if (!isNaN(t)) {
      ...
      if (hours > 0) {
        ...
        if (timeRef.current) timeRef.current.style.width = "42px";
      }
  ...
  const handleDragStart = (_e: RndDragEvent, _d: DraggableData) => {
