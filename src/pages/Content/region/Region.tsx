...
const ResizableBox = () => {
  const regionRef = useRef<Rnd | null>(null);
  ...
  const handleMove = (_e: unknown, d: { x: number; y: number }) => {
    setContentState((prevContentState) => ({
      ...prevContentState,
      regionX: d.x,
      regionY: d.y,
  ...
  useEffect(() => {
    ...
    const forwardToWindow = (e: MouseEvent) => {
      if (
        (e.target as HTMLElement)?.id === shadowHostId ||
        (e.target as HTMLElement)?.closest?.("#" + shadowHostId)
      ) {
  ...
  return (
    <div
      ...
      onClick={(e) => {
        ...
        if (
          className.indexOf("resize-handle") === -1 &&
          className.indexOf("react-draggable") === -1 &&
          className.indexOf("region-rect") === -1
        ) {
        }
      }}
      ref={parentRef}
