...
type ShapeInstance =
  | InstanceType<typeof fabric.Rect>
  | InstanceType<typeof fabric.Triangle>
...
interface ShapeCanvasEvent {
  e: Event;
}

interface ShapeCanvas {
  getPointer(event: Event): { x: number; y: number };
  add(shape: ShapeInstance): void;
  requestRenderAll(): void;
  setActiveObject(shape: ShapeInstance): void;
  on(event: string, listener: (event: ShapeCanvasEvent) => void): void;
  off(event: string, listener: (event: ShapeCanvasEvent) => void): void;
}

type SetShapeState = (state: ShapeToolState) => void;
...
const ShapeTool = (
  ...
  const makeShape = (state: ShapeToolState): ShapeInstance => {
  ...
  const onMouseDown = (o: ShapeCanvasEvent): void => {
    ...
    shape = makeShape(state);
    canvas.add(shape);
    canvas.requestRenderAll();
  };

  const onMouseMove = (o: ShapeCanvasEvent): void => {
    const state = getState();
    if (!state?.drawingMode) return;
    if (state.tool !== "shape") return;
