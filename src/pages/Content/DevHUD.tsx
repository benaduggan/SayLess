...
interface DevHudContentState {
  openToast?: (message: string, onClose: () => void, timeout?: number) => void;
...
const DevHUD: React.FC<DevHudProps> = ({
  ...
  const toast = (msg: string, ms = 8000): void => {
    contentStateRef.current?.openToast?.(msg, () => {}, ms);
  };

  const actions: DevHudAction[] = [
    {
