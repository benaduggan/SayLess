...
const Shortcuts = ({ shortcuts }: { shortcuts?: boolean }) => {
  ...
  useEffect(() => {
    ...
    const shouldHandleShortcut = (event: KeyboardEvent) => {
      const state = contentStateRef.current;
    ...
    const getDigitKey = (event: KeyboardEvent) => {
    ...
    const setTool = (tool: string, nextShape: Record<string, unknown> = {}) => {
    ...
    const deriveCursorMode = (effects: string[], fallback: string) => {
  ...
  useEffect(() => {
    const isPttChord = (event: KeyboardEvent) =>
