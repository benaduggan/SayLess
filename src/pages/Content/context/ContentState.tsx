...
const initialSetTimer: React.Dispatch<React.SetStateAction<number>> = () => {};
export const contentStateContext = createContext<ContentStateContextValue>([
  {},
  initialSetContentState,
  0,
  initialSetTimer,
]);
...
const deriveCursorMode = (effects: string[], fallbackMode?: string) => {
...
const ContentState = (props: React.PropsWithChildren) => {
  ...
  const startBeepRef = useRef<any>(null);
  const stopBeepRef = useRef<any>(null);
  const prevRecordingRef = useRef<any>(null);
  const hydratedRef = useRef(false);
  const suppressStopBeepRef = useRef(false);
  const suppressStartBeepRef = useRef(false);
  const tabIdRef = useRef<any>(null);
  const activeTabRef = useRef<any>(null);
  const tabRecordedIdRef = useRef<any>(null);
  const recordingUiTabRef = useRef<any>(null);
  const recordingStartTimeRef = useRef<any>(null);
  ...
  const pausedAtRef = useRef<any>(null);
  const totalPausedMsRef = useRef(0);
  const recordingFlagRef = useRef(false);
  const timerReadSeqRef = useRef(0);
  const lastBeepStartTimeRef = useRef<any>(null);
  const recordingBeepTabIdRef = useRef<any>(null);
    ...
    (async () => {
      ...
      if ("VideoEncoder" in window) {
        try {
          ...
          realSupport = Boolean(support.supported);
        } catch {}
      }
  ...
  const pauseRecording = useCallback((dismiss?: boolean) => {
  ...
  const startStreaming = useCallback(async () => {
    ...
    if (isExtensionPage) {
      ...
      permission = Boolean(await permissionPromise);
    }
  ...
  const handleDevicePermissions = (data: any) => {
  ...
  const [contentState, setContentStateInternal] = useState<ContentStateValue>({
    color: "#4597F7",
    strokeWidth: 2,
    drawingMode: false,
    tool: "pen",
  ...
  const updateTimerFromStorage = useCallback(async () => {
    ...
    totalPausedMsRef.current = Number(totalPausedMs) || 0;
    tickTimerFromRefs();
  }, [tickTimerFromRefs]);
        ...
        if (mutation.type === "childList") {
          ...
          addedNodes.forEach((node) => {
            if (node.nodeType === Node.ELEMENT_NODE) {
              (node as Element).classList.add("sayless-scrollbar");
            }
          });

          removedNodes.forEach((node) => {
            if (node.nodeType === Node.ELEMENT_NODE) {
              (node as Element).classList.remove("sayless-scrollbar");
            }
          });
        }
      }
