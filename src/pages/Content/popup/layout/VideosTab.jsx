...
const formatBytes = (value) => {
  ...
  return `${amount >= 10 || index === 0 ? Math.round(amount) : amount.toFixed(1)} ${
    units[index]
  }`;
};
...
const buildRecordingMeta = (video) => {
  const parts = [
...
const VideosTab = (props) => {
  ...
  const handleCopyLocalInfo = (video) => {
    const lines = [
  ...
  const handleDuplicate = async (video) => {
    try {
  ...
  const handleExport = async (video, { preferPicker = false } = {}) => {
    try {
  ...
  const downloadBlob = async (blob, fileName, { preferPicker = false } = {}) =>
  ...
  const handleRepair = async (video) => {
    const ok = window.confirm(
            ...
            {visibleVideos.map((video) => {
              ...
              return (
                <VideoItem
                  ...
                  status={
                    isBroken
                      ? { kind: "warning", label: `Missing media: ${health.status}` }
                      : null
                  }
                  ...
                  onDuplicate={isBroken ? null : () => handleDuplicate(video)}
                  onExport={isBroken ? null : () => handleExport(video)}
                  onSaveToFile={
                    isBroken || !canSaveToFile
                      ? null
                      : () => handleExport(video, { preferPicker: true })
                  }
                  onRepair={isBroken ? () => handleRepair(video) : null}
