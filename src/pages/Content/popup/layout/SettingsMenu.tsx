...
const SettingsMenu = (props: SettingsMenuProps) => {
  ...
  const [fastRecorderInfo, setFastRecorderInfo] = useState<any>({
    status: null,
    probe: null,
    decision: null,
    disabled: false,
  ...
  const hasLadderFields = (probe: any) => {
  ...
  useEffect(() => {
    let canceled = false;
    (async () => {
      const status: any = await migrateFastRecorderStatus();
      if (canceled) return;
      setFastRecorderInfo((prev: any) => ({
        ...prev,
        status,
