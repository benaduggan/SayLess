...
import styles from "../../styles/edit/_TrimUI.module.css";
...
const TrimUI = () => {
  ...
  return (
    <div
      className={`${styles.trimWrap} trimWrap`}
      style={{ padding: "0 24px 20px", boxSizing: "border-box" }}
    >
      <div style={header}>
        <span style={{ fontWeight: 600 }}>Timeline</span>
        <span style={{ color: "#888", fontSize: 12 }}>
          {resolved.segments.length} clip{resolved.segments.length === 1 ? "" : "s"} ·{" "}
          {toTimeStamp(outDur)}
        </span>
        <span style={{ flex: 1 }} />
        {hasEdits && (
          <>
            <button
              ...
              disabled={exporting}
              data-testid="timeline-reset-edits"
            >
              Reset
            </button>
            <button
