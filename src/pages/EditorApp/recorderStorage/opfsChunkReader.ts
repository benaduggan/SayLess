...
const chromeApi = (): ReaderChromeApi =>
  (globalThis as typeof globalThis & { chrome: ReaderChromeApi }).chrome;
...
export class OpfsChunkReader implements ChunkReader {
  ...
  private async _readRefState(): Promise<{
    sameFile: boolean;
    finalized: boolean;
  }> {
    try {
  ...
  async readBlob({
    onSlowFinalize,
  }: { onSlowFinalize?: () => void } = {}): Promise<ChunkReadResult> {
    if (!this._opened) {
      throw new Error("opfs-chunk-reader-not-opened");
    }
    const fileName = this._fileName;
    if (!fileName) throw new Error("opfs-chunk-reader-no-filename");
