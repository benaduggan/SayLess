...
const isFiniteTime = (value) => Number.isFinite(Number(value));
...
const makeLabel = (words, startTime, fallback) => {
  const label = words
...
const normalizeTime = (time, duration = Infinity) =>
  Math.max(0, Math.min(Number(time) || 0, duration));

const markerId = (source, time) =>
  `chapter-${source}-${Math.round((Number(time) || 0) * 1000)}`;

const pushMarker = (markers, marker, minSectionSeconds) => {
...
export const normalizeChapterMarkers = (
  ...
  const normalized = markers
    .map((marker, index) => {
      ...
      return {
        id: marker.id || markerId(marker.source || "manual", `${time}-${index}`),
        time,
...
export const buildChapterMarkers = ({
  ...
  for (const event of activityEvents) {
    ...
    pushMarker(
      markers,
      {
        id: markerId(event.type || "activity", time),
        time,
