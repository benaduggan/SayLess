...
/**
 * @typedef {{ type: "mute"|"cut", start: number, end: number }} EdlOp
 */
 ...
 * @returns {EdlOp[]}
 */
export function planEdlOps(edl) {
  ...
  return [
    ...mutes.map((r) => ({ type: "mute", start: r.start, end: r.end })),
    ...deletes.map((r) => ({ type: "cut", start: r.start, end: r.end })),
  ];
}

/** True if the EDL has no edits (caller can skip rendering and keep the source). */
export function isEmptyEdl(edl) {
 ...
 */
export async function applyEdl(sourceBlob, edl, deps) {
  ...
  for (const op of ops) {
    const stepProgress = (p) => deps.onProgress?.((done + (p || 0)) / ops.length);
    if (op.type === "mute") {
