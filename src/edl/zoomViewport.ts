...
export interface ZoomViewportTransform {
  scale: number;
  dx: number;
  dy: number;
  drawWidth: number;
...
export interface ZoomCssTransform {
  transform: string;
  transformOrigin: "0 0";
}

export function clampZoomRatio(value: unknown, fallback = 0.5): number {
...
export function computeZoomViewportTransform(
  zoom: ZoomViewportInput | null | undefined,
  width: number,
  height: number,
...
export function zoomTransformToCss(
  transform: Pick<ZoomViewportTransform, "scale" | "dx" | "dy"> | null,
  width = 100,
  height = 100,
): ZoomCssTransform {
