...
const TEMPLATE = {
  ...
  offlineTranscription: {
    ...
    externalNetworkProbe: {
      ...
      result: "failed",
      observedError:
        "Replace with the browser fetch/ping error observed in the same Chrome profile.",
...
const automatedEvidenceCanPrefillTemplate = (automatedEvidence) => {
  if (
    automatedEvidence?.kind !== "sayless.releaseQaAutomated" ||
    automatedEvidence?.status !== "passed"
  ) {
    return false;
  }
  ...
  if (
    ...
    !isCanonicalRelativePath(automatedEvidence.build.path, "build")
  ) {
    return false;
  }
...
const validateAutomatedEvidence = (evidence, errors) => {
  ...
  if (
    !automatedEvidence?.git?.workingTree ||
    typeof automatedEvidence.git.workingTree !== "object"
  ) {
