...
const ROOT = process.env.SAYLESS_MANUAL_QA_MEASUREMENTS_ROOT
  ...
  : DEFAULT_ROOT;
const EVIDENCE_PATH = join(ROOT, "release-artifacts", "manual-qa-evidence.json");
const MEDIA_REPORT_PATH = join(ROOT, "release-artifacts", "manual-qa-media-probe.json");
const SIDECAR_REPORT_PATH = join(ROOT, "release-artifacts", "manual-qa-sidecar-probe.json");

const args = process.argv.slice(2);
const shouldWrite = args.includes("--write");
const asJson = args.includes("--json");
const help = args.includes("--help") || args.includes("-h");
const unknown = args.filter((arg) => !["--write", "--json", "--help", "-h"].includes(arg));

const usage = () => {
  console.log("Usage: npm run qa:release:manual:measurements -- [--json] [--write]");
  console.log(
...
try {
  ...
  const sidecarReport = readJson(SIDECAR_REPORT_PATH, "manual QA sidecar report");
  ...
  if (shouldWrite && result.changes.length) {
    ...
    writeFileSync(temporaryPath, `${JSON.stringify(result.evidence, null, 2)}\n`);
    renameSync(temporaryPath, EVIDENCE_PATH);
  }
  const report = {
    kind: "sayless.manualQaMeasurementImport",
    status: shouldWrite ? (result.changes.length ? "applied" : "unchanged") : "preview",
  ...
  if (asJson) {
    ...
    if (!shouldWrite) {
      console.log("Preview only. Rerun with --write to update the template atomically.");
    }
