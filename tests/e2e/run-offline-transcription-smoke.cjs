/**
 * Slow smoke harness for full offline Whisper startup/inference.
 *
 * This is separate from qa:release:auto because it loads the 76 MB bundled
 * model and runs actual inference in Chrome. It generates a tiny silent WAV,
 * serves bundled model + ORT assets locally, and verifies the transcription
 * engine completes without network model downloads.
 */
...
(async () => {
  const work = fs.mkdtempSync(path.join(os.tmpdir(), "sl-offline-asr-"));
  ...
  try {
    ...
    result = await page.evaluate(async (baseUrl) => {
      ...
      return {
        ...
        words: Array.isArray(transcript.words) ? transcript.words.length : -1,
  ...
  console.log("=== OFFLINE TRANSCRIPTION SMOKE ===");
