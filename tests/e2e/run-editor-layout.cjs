 ...
 * tests the production CSS class contract instead of only unit-testing style
 * strings.
 */
...
const assertLayout = (name, data) => {
  ...
  assert(data.timeline.height >= 176, `${name}: timeline is too short to operate`, data.timeline);
