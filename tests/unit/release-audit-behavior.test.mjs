...
const makeFixture = ({
  ...
  const defaultScripts = {
    ...
    "release:cws": "npm run build:cws && npm run verify:cws-package && npm run preflight:cws",
    ...
    "qa:release:manual:progress": "node scripts/verify-manual-qa-evidence.mjs --progress",
    ...
    "qa:release:manual:measurements": "node scripts/apply-manual-qa-measurements.mjs",
  ...
  writeJson(join(dir, "package.json"), {
    ...
    devDependencies: {
      ...
      oxlint: "^1.75.0",
      tailwindcss: "^4.3.3",
      ...packageJsonOverrides.devDependencies,
    },
    ...Object.fromEntries(
      Object.entries(packageJsonOverrides).filter(
        ([key]) => key !== "scripts" && key !== "devDependencies",
      ),
    ),
  });
  ...
  writeJson(join(dir, ".oxlintrc.json"), {});
  writeJson(join(dir, ".oxfmtrc.json"), {});
  writeFileSync(join(dir, "postcss.config.mjs"), "export default {};\n");
  mkdirSync(join(dir, "src", "styles"), { recursive: true });
  writeFileSync(join(dir, "src", "styles", "tailwind.css"), "@theme {}\n");
  writeFileSync(
...
`,
  ...
  writeFileSync(
    ...
    'const REQUIRED_AUTOMATED_COMMANDS = ["lint", "format:check", "typecheck"];\n' +
...
test("release audit rejects legacy Screenity debug and keepalive names", () => {
  ...
  try {
    ...
    assert.notEqual(result.status, 0);
    assert.match(result.stderr, /src\/pages\/Recorder\/Recorder\.tsx: SCREENITY_DEBUG_RECORDER/);
...
test("release audit rejects stale active Screenity UI and debug names", () => {
  ...
  try {
    ...
    assert.notEqual(result.status, 0);
    assert.match(result.stderr, /src\/pages\/EditorApp\/EditorApp\.tsx: screenity-scrollbar/);
...
test("release audit rejects retired destructive editor utility paths", () => {
  ...
  try {
    ...
    assert.notEqual(result.status, 0);
    assert.match(result.stderr, /src\/pages\/Editor\/utils\/cropVideo\.ts: file exists/);
    assert.match(result.stderr, /obsolete destructive editor compatibility path\(s\) found/);
  } finally {
    rmSync(fixture, { recursive: true, force: true });
  }
});
...
test("release audit requires packaged editor save cancellation and retry proof", () => {
  ...
  try {
    ...
    assert.match(result.stderr, /export delivery contract\/proof invariant\(s\) missing/);
  } finally {
    rmSync(fixture, { recursive: true, force: true });
  }
});
...
test("release audit rejects remote telemetry and analytics source references", () => {
  const fixture = makeFixture({
    sourceText: "export const eventSink = 'remote analytics telemetry must stay out';\n",
  });
  try {
    const result = runAudit(fixture);
...
`,
    ...
    assert.match(result.stderr, /src\/assets\/editor\/index\.html: <title>React App<\/title>/);
...
test("release audit rejects stale dev auto-reload dependencies", () => {
  ...
  try {
    ...
    writeFileSync(join(fixture, "utils", "server.cts"), 'require("ssestream");\n');
    writeFileSync(
...
test("release audit protects probe-report routing in manual QA progress", () => {
  const fixture = makeFixture();
  try {
    const verifierPath = join(fixture, "scripts", "verify-manual-qa-evidence.mjs");
    ...
    assert.notEqual(result.status, 0);
    assert.match(result.stderr, /must enforce canonical automated and manual-probe provenance/);
  } finally {
    rmSync(fixture, { recursive: true, force: true });
  }
});
...
test("release audit protects measurement-import routing in manual QA progress", () => {
  ...
  try {
    ...
    writeFileSync(
      verifierPath,
      readFileSync(verifierPath, "utf8").replace("MEASUREMENT_IMPORT_ERROR_PATTERN\n", ""),
    );
...
test("release audit protects the guarded manual QA measurement import", () => {
  const fixture = makeFixture();
  try {
    const importerPath = join(fixture, "scripts", "manual-qa-measurement-import.mjs");
    writeFileSync(
      importerPath,
      readFileSync(importerPath, "utf8").replace("measurable-set-complete\n", ""),
    );

    const result = runAudit(fixture);

    assert.notEqual(result.status, 0);
    assert.match(result.stderr, /manual QA measurement import must remain preview-first/);
  } finally {
    rmSync(fixture, { recursive: true, force: true });
  }
});
...
test("release audit protects manual QA section work targets", () => {
  ...
  try {
    const verifierPath = join(fixture, "scripts", "verify-manual-qa-evidence.mjs");
    writeFileSync(verifierPath, readFileSync(verifierPath, "utf8").replace("workTargets\n", ""));
...
test("release audit requires status actions to launch the clean profile", () => {
  ...
  try {
    ...
    writeFileSync(statusPath, readFileSync(statusPath, "utf8").replaceAll(" --launch", ""));
...
test("release audit protects validated active manual session discovery", () => {
  ...
  try {
    ...
    writeFileSync(
      statusPath,
      readFileSync(statusPath, "utf8").replace("discoverActiveManualQaSession\n", ""),
    );
...
test("release audit protects provenance-marked manual profile resume", () => {
  ...
  try {
    ...
    assert.notEqual(result.status, 0);
    assert.match(result.stderr, /permit only provenance-marked session resumption/);
  } finally {
    rmSync(fixture, { recursive: true, force: true });
  }
});
...
test("release audit requires launch acknowledgement before session recording", () => {
  ...
  try {
    const profilePath = join(fixture, "scripts", "manual-qa-profile.mjs");
    writeFileSync(profilePath, readFileSync(profilePath, "utf8").replace("launchChrome\n", ""));
...
test("release audit protects portable manual session provenance stamping", () => {
  ...
  try {
    ...
    writeFileSync(
      profilePath,
      readFileSync(profilePath, "utf8").replace("writeManualSessionProvenance\n", ""),
    );
...
test("release audit rejects a required tester email placeholder", () => {
  ...
  try {
    ...
    assert.notEqual(result.status, 0);
    assert.match(result.stderr, /named tester attribution without requiring contact data/);
  } finally {
    rmSync(fixture, { recursive: true, force: true });
  }
});
